...
def create_event(summary: str, description: str, date: datetime.date, dtstamp: datetime) -> Event:
    ...
    event.add('transp', 'TRANSPARENT')  # Marks the event as free
    event.add('status', 'CONFIRMED')    # Marks the event as confirmed
    return event


def add_timezone_component(calendar: Calendar) -> None:
    """Add UTC timezone component to the calendar."""
    tz = Timezone()
    tz.add('TZID', 'UTC')
    standard = TimezoneStandard()
    standard.add('DTSTART', datetime(1970, 1, 1, tzinfo=timezone.utc))
    standard.add('TZOFFSETFROM', timedelta(hours=0))
...
def generate_ics_for_year(year: int, solstice_data: dict) -> Calendar:
    ...
    add_timezone_component(calendar)  # Add timezone component to the calendar
    ...
    dtstamp = datetime.now(timezone.utc)  # Current timestamp for DTSTAMP
...
def write_to_file(file_name: str, calendar: Calendar):
...
def write_split_ics_files(start_year: int, end_year: int, base_name: str, size_limit: int, solstice_file: str):
    ...
    add_timezone_component(combined_calendar)
    ...
    for year in tqdm(range(start_year, end_year + 1), desc="Processing Years", unit="year"):
        ...
        if current_size + year_size > size_limit:
            write_to_file(f"{base_name}_part{counter}_{current_start_year}-{year - 1}.ics", combined_calendar)
            combined_calendar = Calendar()
            combined_calendar.add('prodid', '-//Redux//EN')
            combined_calendar.add('version', '2.0')
            combined_calendar.add('calscale', 'GREGORIAN')
    ...
    if len(combined_calendar.subcomponents) > 0:
        write_to_file(f"{base_name}_part{counter}_{current_start_year}-{end_year}.ics", combined_calendar)
